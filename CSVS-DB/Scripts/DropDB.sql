...
IF EXISTS (SELECT name FROM master.dbo.sysdatabases WHERE name = N'Commerce')
BEGIN
	DECLARE @spid smallint
	DECLARE @sql varchar(4000)

	DECLARE crsr CURSOR FAST_FORWARD FOR
		SELECT spid FROM sysprocesses p INNER JOIN sysdatabases d ON d.[name] = 'Commerce' AND p.dbid = d.dbid
